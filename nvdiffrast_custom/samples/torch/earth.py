...
# Example usage:
# save_images_to_video('/path/to/image_folder', 'output_video.mp4')
...
def fit_earth(max_iter          = 20000,
    ...
    writer = imageio.get_writer(f'{out_dir}/result_earth.mp4', fps=60)
    for it in range(max_iter + 1):
        ...
        if display_image or save_image:
            ...
            with torch.no_grad():
                ...
                # debug()
                result_image[result_image.sum(axis=-1) < 0.01] = 1
                #make image brighter
                result_image = np.clip(result_image * 1.5, 0, 1)
                writer.append_data((result_image * 255).astype(np.uint8))
                if display_image:
...
def main():
    ...
    # Run.
    fit_earth(max_iter=args.max_iter, log_interval=10, display_interval=args.display_interval, enable_mip=args.mip, out_dir=out_dir, log_fn='log.txt', texsave_interval=1000, texsave_fn='tex_%06d.png', imgsave_interval=1, imgsave_fn='img_%06d.png', use_opengl=args.opengl)
