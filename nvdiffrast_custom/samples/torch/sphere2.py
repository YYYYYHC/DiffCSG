...
# Redefine initial vertices and faces, using lists.
        ...
        n = res
        saved = os.path.exists(f'./triangles_sphere{res}.npy')
        if not saved:
            ...
            np.save(f'./triangles_sphere{res}.npy', self.pos_idx.cpu().numpy())
            np.save(f'./vertices_sphere{res}.npy', self.vtx_pos.cpu().numpy())
            np.save(f'./vertex_normals_sphere{res}.npy', self.vtx_normal.cpu().numpy())
        else:
            triangles_load = np.load(f'./triangles_sphere{res}.npy')
            vertices_load = np.load(f'./vertices_sphere{res}.npy')
            normals_load = np.load(f'./vertex_normals_sphere{res}.npy')
        
            self.vtx_pos = torch.tensor(vertices_load, dtype=torch.float32)
            self.vtx_normal = torch.tensor(normals_load, dtype=torch.float32)
            #extend the normal to 4 dimension
            # self.vtx_normal = torch.cat([self.vtx_normal, torch.zeros(self.vtx_normal.shape[0],1)], dim=1)
        ...
        print("RES:",res, "Sphere tris: ", self.pos_idx.shape)
        if use_cuda:
