...
RasterizeGLStateWrapper::RasterizeGLStateWrapper(bool enableDB, bool automatic_, int cudaDeviceIdx_, bool use_lines)
{
    ...
    rasterizeInitGLContext(NVDR_CTX_PARAMS, *pState, cudaDeviceIdx_, use_lines);
    releaseGLContext();
}
...
std::tuple<torch::Tensor, torch::Tensor> rasterize_fwd_gl(RasterizeGLStateWrapper& stateWrapper, torch::Tensor pos, torch::Tensor tri, std::tuple<int, int> resolution, torch::Tensor ranges, int peeling_idx)
{
    ...
    bool use_lines = s.use_lines;
    // if(use_lines){
    //     printf("using lines mode\n");
    // }
    // Check inputs.
    ...
    if(use_lines)
    {
        NVDR_CHECK(tri.sizes().size() == 2 && tri.size(0) > 0 && tri.size(1) == 2, "line must have shape [>0, 2]");
    }
    else
    {
    ...
    if(use_lines){
        rasterizeRender_lines(NVDR_CTX_PARAMS, s, stream, posPtr, posCount, vtxPerInstance, triPtr, triCount, rangesPtr, width, height, depth, peeling_idx);
    }
    else{
