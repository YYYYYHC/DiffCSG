...
static void constructGLProgram(NVDR_CTX_ARGS, GLuint* pProgram, GLuint glVertexShader, GLuint glGeometryShader, GLuint glFragmentShader, bool use_lines)
{
    *pProgram = 0;
...
void rasterizeInitGLContext(NVDR_CTX_ARGS, RasterizeGLState& s, int cudaDeviceIdx, bool use_lines)
{
    ...
    // Finalize programs.
    constructGLProgram(NVDR_CTX_PARAMS, &s.glProgram, s.glVertexShader, s.glGeometryShader, s.glFragmentShader,use_lines);
    constructGLProgram(NVDR_CTX_PARAMS, &s.glProgramDP, s.glVertexShader, s.glGeometryShader, s.glFragmentShaderDP, use_lines);
...
void rasterizeRender_lines(NVDR_CTX_ARGS, RasterizeGLState& s, cudaStream_t stream, const float* posPtr, int posCount, int vtxPerInstance, const int32_t* triPtr, int triCount, const int32_t* rangesPtr, int width, int height, int depth, int peeling_idx)
{
    ...
    if (depth == 1 && !rangesPtr)
    {
        ...
        // Trivial case.
        NVDR_CHECK_GL_ERROR(glDrawElements(GL_LINES, triCount, GL_UNSIGNED_INT, 0));
    }
